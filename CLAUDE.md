...
Static Hebrew law-firm website for attorney Tzachi Lavia (עו"ד צחי לביא), specializing in personal injury (נזיקין) and medical malpractice (רשלנות רפואית). Based in Ashkelon, Israel. 17+ years experience.
...
## Required Context Files (read before writing content)

Before drafting any copy, article, email, social post, or client-facing text, read the following files for voice, tone, terminology, and personal context:

1. **`.claude/brand-voice-guidelines.md`** — Brand voice playbook: 7 voice attributes, 5 messaging pillars, tone-by-context matrix, terminology (Must-Use / Avoid / Never-Use), compliance requirements. Apply these rules to every piece of content.
2. **`.claude/about-me.md`** — Personal context about Tzachi: background, working style, communication preferences, current priorities, and interaction rules. Use this to tailor how you collaborate, not what the brand says publicly.

These files are mandatory context. If either is missing or appears outdated, flag it before continuing.
